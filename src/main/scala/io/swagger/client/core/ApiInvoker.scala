...
object ApiInvoker {
  ...
  def addCustomStatusCode(code: Int, reason: String = "Application defined code", isSuccess: Boolean = true) = {
    StatusCodes.getForKey(code) foreach { c =>
      StatusCodes.custom(code, reason, reason, isSuccess, allowsEntity = true)
    }
  }
...
class ApiInvoker(formats: Formats)(implicit system: ActorSystem) extends CustomContentTypes {
      ...
      case HttpMethods.POST | HttpMethods.PUT =>
        formDataContent(request) orElse bodyContent(request) match {
          ...
          case Some(c: Multipart.FormData) =>
            builder.apply(uri, c)
          case Some(c: String) =>
            builder.apply(uri, HttpEntity(normalizedContentType(request.contentType), ByteString(c)))
          case _ =>
            builder.apply(uri, HttpEntity(normalizedContentType(request.contentType), ByteString(" ")))
        }
    ...
    request.responseForCode(response.status.intValue) match {
      case Some((manifest: Manifest[T], state: ResponseState)) =>
        entityUnmarshaller(manifest)(response.entity) match {
          ...
          case Left(UnmarshallingError.ContentExpected) ⇒
...
sealed trait CustomContentTypes {
  ...
  def parseContentType(contentType: String): ContentType = {
    val contentTypeAsRawHeader = RawHeader("Content-Type", contentType)

    (contentTypeAsRawHeader: @unchecked) match {
      case Right(ct: HttpHeader.`Content-Type`) =>
        ct.contentType
